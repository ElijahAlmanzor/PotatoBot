...
def main():
    ...
    for num in range(1,16):
        try:
            name = str(num) + ".npy"
            load_path = os.path.join(os.path.expanduser('~'), 'pp_ws', 'src', 'planter', 'planter_kg_control', 'traj_vertical', name)
    ...
    fig.suptitle('Vertical Picking Coordinates (X,Y,Z) against Time (s)')
    fig2 = plt.figure()
